# -*- coding: utf-8 -*-
"""
Created on Wed Apr  3 08:04:25 2024

@author: keshengxu@gmail.com
"""
...
# the difference between  simulations time (t) and  times of last spikes
interval_ex = np.zeros(Ne)
           

# the tempering variable 
tmp_ex = np.zeros(Ne)
 

# initial conditions of ux, x,u
ux_ex = np.zeros(Ne)
...
tau_rec = 500 #500 #750 #ms depress
tau_facil = 2000 #2000 #50 # ms facilitaion
    # tuas = 20 #ms
...
record_y = np.zeros((Ne, nsteps), dtype=dtype)
 ...
 # array for recording data of k and (k+1) steps
...
Y_t = np.zeros((nsteps, N), dtype=np.float32)  # saving Y_t
Z_t = np.zeros((nsteps, N), dtype=np.float32)  # saving Z_t
...
if isim%threads==rank:
        ...
        net = Networkmodel(Ne1 =Ne,   # The numbers of excitatroy ensemble
          ...
          W_ie1 = jei, # the weight of excitatory currents in inhibitory population from excitatory population
          )
    ...
    # the gap junction matrix 
        ...
        for i in range(nsteps):
            ...
            interval_ex = t - lastSpike_ex  # the time difference between simulation and last spikes times for excitatory neurons
           
            
            #Short-term plasticity updating rule  without spikes
            ...
            if i >=1:
                for ci in range (Ne): #  looping over EX neurons
                    if V_t[i, ci] >= -0.1 and  V_t[i-1, ci] < -0.1: # spike occurrence
                        x_ex[ci]  *= tmp_ex[ci]
                        u_ex[ci]  += U * tmp_ex[ci]
                        lastSpike_ex[ci] = t
            ux_ex = u_ex * x_ex 
            ...
            ISyn_t[i,:] = (sey + seye) * (V_t[i,:] - VsynE) + siy* (V_t[i,:] - VsynI)
        ...
        Y_t=Y_t[equil_dt:,:]
        Z_t=Z_t[equil_dt:,:]
        ISyn_t=ISyn_t[equil_dt:,:]
        Time_simu= Time2
        
        
        
neuron_idx = 49
np.savetxt(f"ISyn_neuron50_jee_{jee}_jei_{jei}.txt", ISyn_t[:, neuron_idx], fmt='%.6f')
np.savetxt(f"X_neuron50_jee_{jee}_jei_{jei}.txt", V_t[:, neuron_idx], fmt='%.6f') 
